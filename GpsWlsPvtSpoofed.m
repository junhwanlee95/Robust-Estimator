function [gnssMeas, SatStateOutlier, xStateOutlier,  WPOutlier, WROutlier ,SpoofedSignal, NormalSignal] = GpsWlsPvtSpoofed(gnssMeas,allGpsEph)
...
% acceleration = [0.95*ones(1,10) -0.95*ones(1,20) 0.3*ones(1,25)  0.7*ones(1,25) -0.5*ones(1,20) 0.5*ones(1,30)   0.5*ones(1,25) 1];
acceleration = 5*ones(1,80);



acceleration = [zeros(1,attacktime) acceleration+0.01*rand(1,length(acceleration)),  0.01*rand(1,N-length(acceleration)-attacktime)];
...
% attackvel=[zeros(1,attacktime) 300*ones(1, N-attacktime)]';
...
alpha=0;
AttackonPseudorange = tril(ones(length(attackvel),length(attackvel)))*attackvel;
AttackonPseudorangeRates = [0 ; D*AttackonPseudorange] ;
% AttackonPseudorangeRates = [0 ; zeros(490,1)] ;

%% Manual Attack
% AttackonPseudorange = [];
% AttackonPseudorangeRates =[];
% for j = 1:N
%     if j > 30 && j < 50
%         d1 = 30;
%         d2 = 10;
...
%         d2 = 0;
%     end
% 	AttackonPseudorange = [AttackonPseudorange;d1]; 
% %     AttackonPseudorange = zeros(N,1);
%     AttackonPseudorangeRates = [AttackonPseudorangeRates;d2];
% %     AttackonPseudorangeRates = zeros(N,1);
%     
% end

%% Creating Attack Signal
AttackSignal = [AttackonPseudorange AttackonPseudorangeRates];
...
%         if (i<=attacktime || i>=50)
    if (i<attacktime)
        ...
        gnssMeas.PrrMpsOutlier(i,:)= gnssMeas.PrrMps(i,:);
    else
...
% % %          gnssMeas.PrMOutlier(i,:)= gnssMeas.PrM(i,:)-attackvel(i);
        ...
        tempNormalSignal = [prM(1:4)' prrMps(1:4)'];
        NormalSignal = [NormalSignal; tempNormalSignal];
        tempSpoofedSignal = [prMWithOutlier(1:4)' prrMpsWithOutlier(1:4)'];
        SpoofedSignal = [SpoofedSignal; tempSpoofedSignal];
